	...
	<head>
    	...
    	<script>
    	        ...
    	        var content = `
    	            <form class="modal-form" onsubmit="sortBooks(event)">
    	                <select id="sortCriteria" name="sortCriteria" required>
    	                    <option value="s2.Arrtime">Arrival Time</option>
    	                    <option value="s1.Deptime">Departure Time</option>
    	                    <option value="ts.TotalFare">Fare</option>
    	                </select>
    	    ...
    	    function sortBooks(event) {
    	        ...
    	        var xhr = new XMLHttpRequest();
    	        xhr.open("POST", "SortBooks.jsp", true);
    	        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    	        xhr.onreadystatechange = function() {
    	            if (xhr.readyState === 4 && xhr.status === 200) {
    	                document.getElementById('bookResults').innerHTML = xhr.responseText;
    	...
    	<div class="bottom-part">
        	<div class="rectangle">
    			<div class="title-container">
        			...
        			<button id="sortButton" onclick="openSortBookModal()">Sort</button>
